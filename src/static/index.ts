const ICONS = {
  account: require('@images/icons/account/account.webp'),
  add: require('@images/icons/add/add.webp'),
  chevronRight: require('@images/icons/chevronRight/chevronRight.webp'),
  products: require('@images/icons/products/products.webp'),
  settings: require('@images/icons/settings/settings.webp'),
  volunteer: require('@images/icons/volunteer/volunteer.webp'),
  down: require('@images/icons/down/down.webp'),
  right: require('@images/icons/right/right.webp'),
  swap: require('@images/icons/swap/swap.webp'),
  refresh: require('@images/icons/refresh/refresh.webp'),
};

const LOGO = {
  32: require('@images/logo/32/logo.webp'),
};

export { ICONS, LOGO };
